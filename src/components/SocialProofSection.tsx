...
export const SocialProofSection = () => {
  return <section className="py-20 bg-background text-foreground">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-black mb-6">
            Echte Menschen,
            <span className="text-foreground"> echte Erfolge</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {successStories.map((story, index) => <Card key={index} className="bg-card backdrop-blur-sm border border-border p-6 hover:bg-muted/50 transition-all duration-300 group">
              <div className="relative mb-4">
                <div className="aspect-video bg-muted rounded-lg flex items-center justify-center relative overflow-hidden">
                  <Play className="w-12 h-12 text-muted-foreground opacity-80 group-hover:opacity-100 transition-opacity" />
                  <div className="absolute top-2 left-2">
                    <Badge variant="secondary" className="text-foreground border-0 text-xs bg-card">
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <a href={story.linkedinUrl} className="text-muted-foreground hover:text-foreground">
                    <ExternalLink className="w-4 h-4" />
                  </a>
                </div>
                <p className="text-sm text-muted-foreground mb-2">{story.profession}</p>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-foreground">{story.salaryBefore}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-muted-foreground">Nachher:</span>
                  <span className="text-foreground font-semibold">{story.salaryAfter}</span>
                </div>
                <div className="flex justify-between items-center text-sm border-t border-border pt-2">
                  <span className="text-muted-foreground">Mehr-Verdienst:</span>
                  <span className="font-bold text-foreground">{story.increase}</span>
                </div>
              </div>

              <p className="text-sm text-muted-foreground mb-3 italic">"{story.testimonial}"</p>
              
              <div className="flex justify-between items-center text-xs text-muted-foreground">
                <span>Dauer: {story.timeframe}</span>
                <div className="flex items-center gap-1 bg-transparent">
                  {[...Array(5)].map((_, i) => <Star key={i} className="w-3 h-3 fill-foreground text-foreground bg-transparent" />)}
                </div>
              </div>
            </Card>)}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
            ...
            <div className="text-center">
              <div className="mb-4">
                ...
                <div className="flex items-center justify-center gap-1 mb-2">
                  {[...Array(5)].map((_, i) => <Star key={i} className={`w-6 h-6 ${i < Math.floor(trustpilotStats.rating) ? 'fill-foreground text-foreground' : 'text-muted-foreground'}`} />)}
                  <span className="ml-2 text-2xl font-bold text-foreground">{trustpilotStats.rating}</span>
                </div>
                <p className="text-muted-foreground mb-4">{trustpilotStats.reviews} Bewertungen</p>
              </div>
              
              <div className="bg-muted rounded-lg p-4 mb-4">
                <div className="text-3xl font-black text-foreground mb-1">{trustpilotStats.excellentPercentage}%</div>
                <div className="text-sm text-muted-foreground">bewerten uns als "Ausgezeichnet"</div>
              </div>
          ...
          <Card className="bg-card backdrop-blur-sm border border-border p-8">
            <div className="text-center mb-6">
              <Badge variant="secondary" className="mb-4 bg-muted text-foreground border-border">
                <Award className="w-4 h-4 mr-2" />
                SRF Schweizer Fernsehen
              </Badge>
              <h3 className="text-xl font-bold mb-2">Wie Deutsche in der Schweiz erfolgreich werden</h3>
              <p className="text-muted-foreground text-sm">Exklusive Reportage über SwissPats</p>
            </div>

            <div className="space-y-2 mb-6">
              {srfVideoChapters.map((chapter, index) => <div key={index} className="flex items-center justify-between p-2 bg-muted/50 rounded hover:bg-muted transition-colors">

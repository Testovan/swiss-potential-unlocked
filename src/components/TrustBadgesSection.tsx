  ...
  return <section className="py-20 bg-gradient-to-br from-off-white to-silver-gray/30">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {trustBadges.map((badge, index) => <Card key={index} className={`p-6 text-center border-2 hover:shadow-soft transition-all duration-300 ${badge.color === 'alpine-green' ? 'border-alpine-green/20 bg-alpine-green/5 hover:border-alpine-green/40' : 'border-burgundy/20 bg-burgundy/5 hover:border-burgundy/40'}`}>
              <div className={`inline-flex p-3 rounded-xl mb-4 ${badge.color === 'alpine-green' ? 'bg-alpine-green/20 text-alpine-green' : 'bg-burgundy/20 text-burgundy'}`}>
                {badge.icon}
              </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {certifications.map((cert, index) => <Card key={index} className="p-6 bg-gradient-to-br from-burgundy/5 to-alpine-green/5 border border-silver-gray bg-white">
                <div className="flex items-start gap-4">
                  <Badge variant="secondary" className="text-burgundy border-burgundy/30 shrink-0 bg-white">
                    {cert.badge}
                  </Badge>
                  <div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 items-center">
            {partnerLogos.map((partner, index) => <a key={index} href={partner.url} className="flex items-center justify-center p-4 bg-off-white rounded-lg border border-silver-gray hover:shadow-soft transition-all duration-300 grayscale hover:grayscale-0">
                <img src={partner.logo} alt={partner.name} className="max-h-8 w-auto opacity-60 hover:opacity-100 transition-opacity" />
              </a>)}
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <Card className="max-w-3xl mx-auto p-8 bg-gradient-to-r from-alpine-green/10 to-burgundy/10 border border-alpine-green/20 bg-zinc-50">
            <div className="space-y-4">
              {guarantees.map((guarantee, index) => <div key={index} className="flex items-center gap-3">
                  {guarantee.icon}
